...
> 注：Go 语言使用完整的 AST 解析，提供最强大的分析能力；其他语言使用基于文本的模式匹配，提供基础但实用的检测功能。
...
- 全局变量检测：识别文件中的全局变量声明
- 可导出变量（大写/public）标记为严重
- 不可导出变量标记为中等

**"我明明刚去过洗手间"** - 重复生成的代码

- 重复函数检测：识别内容完全相同的函数
- 使用哈希算法比对函数内容（忽略空白和格式）
- 2个重复标记为中等，3个以上标记为严重

**"啊？沾衣服上了？"** - 注释掉的代码

- 检测被注释掉的代码（而不是真正的注释）
- 自动排除TODO/FIXME等常见注释标记
- 函数定义标记为严重，变量/常量标记为中等，其他代码标记为低
...
- 检测未使用defer释放的资源（文件、HTTP连接、数据库连接等）
- 自动识别os.Open、http.Get、sql.Open等资源分配
- 文件和HTTP连接标记为严重，数据库连接标记为极其严重

**"金针菇"** - 过于宽泛的类型约束

- 检测函数参数和返回值使用过于宽泛的类型（interface{}、any）
- 识别缺乏类型安全性的函数签名
- 所有宽泛类型使用标记为中等严重程度

**"吓死我了，只是个屁"** - AI生成的不准确常量

- 检测AI生成的常量定义不准确的情况
- 自动比对常见数学常量（PI、E、PHI等）的准确性
- 确认不准确的常量标记为严重程度

**"便秘"** - 缺少异常处理和输入验证

- 检测缺少异常处理、边界条件检查或输入验证
- 识别SQL注入风险（字符串拼接构建SQL、未验证的输入）
- 所有缺少验证的危险操作标记为严重

**"结实"** - 硬编码的配置和敏感信息

- 检测硬编码的配置、URL、API token等敏感信息
- 自动识别URL、数据库连接字符串、密钥等
- URL标记为中等，API token/密钥/数据库连接标记为严重
